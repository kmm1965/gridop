              ...
              double sp_step, size_t size)
{
        ...
        const cyclic_index row_index(size_x), column_index(size_y);
...
/*
inline double Psi_sep(double C){
    return A_psi * sqr(C * (1 - C));
}*/
...
/*            if (fabs(x-x0) + fabs(y-y0) < R)
                 = 1.0;
            else
                cn[index[index_value_type(i, j)]] = 0.0;*/
		}
	}
